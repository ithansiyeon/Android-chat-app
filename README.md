...
<table style="width:500px;">
    ...
    <tr>
        <td>
         RSA 암호화 통신에 있어서 JAVA 언어 기반이므로 암호화된 메시지를 보내기 전 교환이 되어야 하는데 KEY를 패킷 형태로 송수신할 시 인식이 불가능하여 메시지를 암호화하지 못해 통신이 완료되지 못했지만, KEY를 패킷 형태로 보내는 것이 아니라 버퍼 리더로 읽도록 송수신 과정을 새롭게 작성하여 이를 해결할 수 있었습니다. 네트워크와 P2P 채팅에 대해서 잘 이해할 수 있었으며 스레드와 RSA 알고리즘에 대해서 더 자세히 이해할 수 있었습니다. 다음에는 보안성이 강화되도록 KEY 송수신 과정에 다른 암호 알고리즘을 사용하여 제삼자가 파악하기 힘들도록 이중 암호화 과정을 도입하는 방식으로 개선을 하고 싶습니다.
        </td>
    </tr>
</table>

...
<table style="width:500px;">
    ...
    <tr>
        <th>
            RSA 암호화 통신에 있어써 JAVA 언어 기반이므로 암호화된 메시지를 보내기 전 교환이 되어야 하는데 KEY를 패킷 형태로 송수신 할시 인식이 불가능 하여 메시지를
            암호화 하지 못해 통신이 완료되지 못지만 KEY를 패킷 형태로 보내는 것이 아니라 버퍼리더로 읽도록 송수신 과정을 새롭게 작성하여 이를 해결할 수 있었습니다.
            네크워크와 P2P채팅에 대해서 잘 이해할 수 있었습니다. 쓰레드와 RSA 알고리즘에 대해서 더 자세히 이해할 수 있었습니다. 추후에 보안성이 강화되도록 KEY 송수신 
